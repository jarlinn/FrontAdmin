...
interface ModalityDistributionPieChartProps {
  title?: string
  height?: number
}
...
export default function ModalityDistributionPieChart({
  title = "Distribución por Modalidad",
  height = 500
}: ModalityDistributionPieChartProps) {
  ...
  const chartData = data.map((item, index) => ({
    name: item.modality,
    value: item.count,
    percentage: data.length > 0 ? ((item.count / data.reduce((sum, m) => sum + m.count, 0)) * 100).toFixed(1) : '0'
  }))
