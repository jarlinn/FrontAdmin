...
interface SimilarityScoreDistributionVerticalChartProps {
  title?: string
  height?: number
}
...
export default function SimilarityScoreDistributionVerticalChart({
  title = "Distribución por Similarity Score - Barras Verticales",
  height = 500
}: SimilarityScoreDistributionVerticalChartProps) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow-lg max-w-sm">
          ...
          <p className="text-xs text-gray-700 mb-1">{data.range}</p>
          <p className="text-xs text-gray-600">({data.numericRange})</p>
        </div>
      )
    }
    return null
