...
import { useSubmodalityDistribution } from '@/hooks/use-submodality-distribution'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Skeleton } from '@/components/ui/skeleton'

interface SubmodalityDistributionVerticalChartProps {
  title?: string
  height?: number
}
...
export default function SubmodalityDistributionVerticalChart({
  title = "Distribución por Submodalidad - Barras Verticales",
  height = 500
}: SubmodalityDistributionVerticalChartProps) {
  const { data, loading, error, refresh } = useSubmodalityDistribution()
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow-lg max-w-sm">
          <p className="font-medium text-sm mb-1">Submodalidad:</p>
          <p className="text-xs text-gray-700">{data.submodality}</p>
        </div>
      )
    }
    return null
