...
interface ModalityDistributionVerticalChartProps {
  title?: string
  height?: number
}

export default function ModalityDistributionVerticalChart({
  title = "Distribución por Modalidad - Barras Verticales",
  height = 500
}: ModalityDistributionVerticalChartProps) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow-lg max-w-sm">
          <p className="font-medium text-sm mb-1">Modalidad:</p>
          <p className="text-xs text-gray-700">{data.modality}</p>
        </div>
      )
    }
    return null
